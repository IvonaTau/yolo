...
void print_imagenet_detections(FILE *fp, char *id, box *boxes, float **probs, int total, int classes, int w, int h)
{
    ...
    for(i = 0; i < total; ++i){
        ...
        for(j = 0; j < classes; ++j){
            int class = j;
            if (probs[i][class]) fprintf(fp, " %d %f %f %f %f %f\n", j+1, probs[i][class],
                    xmin, ymin, xmax, ymax);
        }
    }
}
...
// box * get_boxes_detector(char *datacfg, char *cfgfile, char *weightfile, char *filename, float thresh, float hier_thresh)
// {
//     FILE * fp;
//     fp = fopen ("bounding_boxes.txt", "w+");
//     printf("we are in the detector!\n");
//     list *options = read_data_cfg(datacfg);
//     char *name_list = option_find_str(options, "names", "data/names.list");
//     char **names = get_labels(name_list);
...
//     printf("Resizing to %i and %i", net.w, net.h);
//     layer l = net.layers[net.n-1];
//     box *boxes = calloc(l.w*l.h*l.n, sizeof(box));
//     float **probs = calloc(l.w*l.h*l.n, sizeof(float *));
...
//     time=clock();
//     network_predict(net, X);
//     printf("%s: Predicted in %f seconds by detector.c.\n", input, sec(clock()-time));
//     get_region_boxes(l, 1, 1, thresh, probs, boxes, 0, 0, hier_thresh);
//     if (l.softmax_tree && nms) do_nms_obj(boxes, probs, l.w*l.h*l.n, l.classes, nms);
//     else if (nms) do_nms_sort(boxes, probs, l.w*l.h*l.n, l.classes, nms);
//     printf("%i boxes length\n", l.w*l.h*l.n );
